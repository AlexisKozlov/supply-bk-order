...
- Produces:
  ...
  //   'col_totals' => array,        // sku => сумма кол-ва (для Telegram «топ-5»)
  //   'products_map' => array,      // sku => ['sku','name'] (имена для «топ-5»)
...
Примечание: поведение Telegram-сводки сохранено полностью, включая блок «топ-5 товаров» (данные `col_totals`/`products_map` приходят из хелпера). Регресс-проверка в Step 4.
