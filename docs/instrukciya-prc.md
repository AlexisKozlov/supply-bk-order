...
**1** — регион и номер точки в ДОДО ИС. Это **ссылка**: нажмите — откроется лист ресторана.
**2** — сколько лотков и сколько листов (стопок) у этого ресторана.
**3** — итог за день: общий объём и сколько страниц печатать.
...
**1** — адрес крупно (по нему ориентируется водитель), под ним номер в ДОДО ИС и дата отгрузки.
**2** — какая это стопка по счёту: **СТОПКА 1 / 11**.
**3** — что кладём: наименование, количество лотков и цвет стикера.
**4** — **ВСЯ ЗАЯВКА**: весь заказ ресторана целиком, чтобы видеть картину.
**5** — подпись листа, дублирует номер стопки.
...
**1** — в чёрной полосе написано **СБОРНАЯ СТОПКА**.
**2** — несколько строк, по строке на каждый размер теста.
**3** — свой стикер у каждой строки: смотрите **построчно**, они разные.
**4** — **ИТОГО В СТОПКЕ**: сколько лотков всего.
