...
Остальные вкладки — «Обзор», «Графики», «Шаблон товаров», «Настройки» — рабочие
экраны отдела закупок. Они у вас открываются, но менять там ничего не нужно
и не получится: сохранение доступно только закупкам.
...
На экране «Приём» есть и кнопки для отдела закупок — «Отправить сводку»,
«На почту поставщику», «+ Довоз», «Напомнить не подавшим», «Закрыть день».
Они вам не нужны: нажатие ничего не изменит, портал ответит отказом.
...
**Нажал кнопку, а портал написал, что нет доступа.**
Значит это кнопка отдела закупок. Ничего не сломалось, просто закройте
сообщение и работайте дальше.
