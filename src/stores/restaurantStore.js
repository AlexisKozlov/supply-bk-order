...
export const useRestaurantStore = defineStore('restaurant', () => {
  ...
  const schedule = ref([]);
  // Подписки ресторанов на напоминания об основной поставке.
  // Карта: restaurant_id → { is_enabled, telegram_enabled, tg_names: [...] }
  // Заполняется параллельно с расписанием. Используется в DeliveryScheduleView
  // для иконки-колокольчика рядом с номером ресторана.
  const mainSubscriptions = ref({});
  ...
  async function _doLoad(group, myId) {
    try {
      ...
      // Подписки на основную поставку зовём параллельно — независимый RPC.
      if (rIds.length) {
        const [sRes, subRes] = await Promise.all([
          db.from('delivery_schedule').select('*').in('restaurant_id', rIds),
          db.rpc('list_main_delivery_subscriptions', { legal_entity_group: group }),
        ]);
        if (myId !== _loadId) return;
        if (sRes.error) throw new Error(sRes.error);
        schedule.value = sRes.data || [];
        // Подписки — некритичны: если RPC упал (например, нет прав), просто
        // оставляем пустую карту и индикатор не показывается.
        mainSubscriptions.value = (subRes && subRes.data && subRes.data.subscriptions) || {};
      } else {
        schedule.value = [];
        mainSubscriptions.value = {};
      }
      loaded.value = true;
  ...
  function mainSubscriptionFor(restaurantId) {
    if (restaurantId == null) return null;
    return mainSubscriptions.value[restaurantId] || mainSubscriptions.value[String(restaurantId)] || null;
  }
