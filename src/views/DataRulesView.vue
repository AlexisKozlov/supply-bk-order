    ...
    <div class="rules-wrap">
      ...
      <div class="rules-grid">
        <section v-for="s in sections" :key="s.title" class="rules-card">
          <div class="rules-card-head">
            <span class="rules-ico">{{ s.icon }}</span>
            <h2>{{ s.title }}</h2>
          </div>
          <p>{{ s.text }}</p>
        </section>
      ...
      <section class="rules-contact">
        <div class="rules-contact-ico">💬</div>
        <div>
...
const sections = [
  { icon: '🎯', title: 'Зачем это нужно', text: 'Данные нужны для входа в портал, разграничения доступа по ролям и юрлицам, сохранения и восстановления рабочих действий, ведения истории изменений (кто, что и когда менял), отправки служебных уведомлений и корректной работы всех модулей: заказов, поставок, остатков, заявок, цен, аналитики и задач. Портал не использует данные для рекламы, профилирования или продажи третьим лицам.' },
  { icon: '🏢', title: 'Разграничение по юрлицам', text: 'Рабочие данные разделены по юрлицам — Бургер БК, Воглия Матта и Пицца Стар. Заказы, остатки, расход, цены и заявки каждого юрлица хранятся и показываются отдельно. Пользователь видит только те юрлица и модули, к которым у него есть доступ; часть модулей (например, связанные с 1С) доступна только для БК и ВМ.' },
  { icon: '👥', title: 'Кто имеет доступ', text: 'Доступ к данным есть только у авторизованных сотрудников. Права выдаются по ролям и по каждому модулю отдельно (просмотр, редактирование, полный доступ). Например, отдел качества и бухгалтерия могут только просматривать справочники, а закупки — редактировать. Кабинеты ресторанов видят только свои данные.' },
  { icon: '📝', title: 'История и аудит', text: 'Ключевые действия записываются в журнал: создание и изменение заказов, заявок, цен, справочников, вход в систему. По многим объектам доступна отмена/повтор действий и просмотр, кто внёс изменение. Это нужно для прозрачности и восстановления данных при ошибках.' },
  { icon: '💾', title: 'Хранение в браузере', text: 'Портал хранит в браузере токен сессии входа, настройки интерфейса, черновики заказов и отметки (например, избранные разделы). Как приложение (PWA) портал может кэшировать интерфейс для быстрой загрузки. Это техническое хранение для работы портала, оно не используется для рекламы или сторонней аналитики.' },
  { icon: '🔔', title: 'Уведомления', text: 'Служебные уведомления приходят на рабочую почту и, по желанию пользователя, в Telegram-бота или как push в приложении. Уведомления касаются только рабочих процессов: заявок, поставок, напоминаний о дедлайнах, изменения цен и статусов. Подписку на Telegram можно подключить и отключить в любой момент.' },
  { icon: '🔐', title: 'Безопасность', text: 'Пароли хранятся в зашифрованном виде (хеширование), вход защищён ограничением числа попыток за короткий период. Сессии имеют срок действия, их можно завершить на всех устройствах разом. Передача данных идёт по защищённому соединению (HTTPS). Доступ к данным — только у пользователей с соответствующими правами.' },
  { icon: '🌐', title: 'Внешние сервисы', text: 'Для отдельных функций портал обращается к внешним сервисам: Telegram (доставка уведомлений и привязка аккаунта), почтовый сервер (отправка писем). Наружу уходит только то, что нужно для конкретной функции, — например, текст уведомления и Telegram ID. Сторонняя реклама и трекеры не используются.' },
  { icon: '🍔', title: 'Кабинеты ресторанов', text: 'Рестораны заходят в свой кабинет по номеру и паролю или по ссылке из Telegram. Для кабинета может храниться рабочий email (для входа и уведомлений) и привязки Telegram сотрудников ресторана. Ресторан видит только свои заявки, остатки и уведомления.' },
  { icon: '🗑️', title: 'Доступ и удаление', text: 'Чтобы уточнить, изменить или удалить данные, отключить Telegram или закрыть доступ — напишите администратору портала. Отключённый ресторан или сотрудник теряет доступ, а связанные уведомления прекращаются.' },
];
</script>
