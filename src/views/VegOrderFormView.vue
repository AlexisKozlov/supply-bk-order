<template>
  <div class="sf-page">
    ...
    <div class="sf-card" v-if="!expired && !submitted">
      <div class="sf-header">
        <div class="sf-badge veg-badge">Планета Ресторанов</div>
      ...
      <template v-else-if="info">
        ...
        <template v-if="selectedRestaurant">
          ...
          <div v-else-if="deliveries.length === 0" class="sf-no-schedule">
            Для этого ресторана не настроен график доставки. Обратитесь к отделу закупок.
          </div>
    ...
    <div class="sf-card" v-else>
      <div class="sf-expired">
        ...
        <template v-if="expiredReason === 'closed'">
          ...
          <p>На данный момент сбор заявок не проводится. Когда начнётся новый сбор, вам пришлют новую ссылку.</p>
        </template>
