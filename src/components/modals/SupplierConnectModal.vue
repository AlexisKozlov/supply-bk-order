    ...
    <div class="scm-modal">
      ...
      <div class="scm-body">
        ...
        <div v-if="step === 0" class="scm-pane">
          ...
          <div v-if="loadingAvailable" class="scm-loading">Загрузка...</div>
        ...
        <div v-else-if="step === 1" class="scm-pane">
          ...
          <div v-if="loadingRests" class="scm-loading">Загрузка ресторанов...</div>
        ...
        <div v-else-if="step === 5" class="scm-pane">
          <p class="scm-hint">Кто из закупщиков получает в Telegram сводку после дедлайна (Excel со всеми заявками ресторанов).</p>
          <div v-if="loadingUsers" class="scm-loading">Загрузка пользователей...</div>
...
async function nextStep() {
  ...
  if (step.value === 2 && !activeTplEntity.value && templateEntities.value.length) {
