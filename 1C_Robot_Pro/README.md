...
Их можно получить прямо в программе:

1. Нажать `Справочник товаров` и выбрать Excel со столбцами `Штрихкод` и `Артикул`.
2. Нажать `Накладная / сводная` и выбрать файл СТТ или сводной таблицы.
3. Выбрать режим `Сводная ЭТТН` или `Одна СТТ`.
4. Нажать `Сформировать output`.
5. Программа очистит старые Excel-файлы в `output` и создаст новые `*_queue_ok.xlsx`.

Также файлы можно получать через сайт:
