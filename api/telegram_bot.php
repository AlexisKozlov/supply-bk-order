...
function cmdOrders($chatId, $user, $editMsgId = null) {
    ...
    if (!$orders) {
        $btns = [
            [['text' => '📦 Заказы на сайте', 'url' => 'https://supply-department.online/orders']],
...
function cmdStock($chatId, $user, $editMsgId = null) {
    ...
    $btns = [
        [['text' => '📊 Анализ запасов', 'url' => 'https://supply-department.online/analysis']],
        [['text' => '◂ Меню', 'callback_data' => 'cmd_menu']],
    ];
...
function cmdPrices($chatId, $user, $editMsgId = null) {
    ...
    $btns = [
        [['text' => '💰 Цены на сайте', 'url' => 'https://supply-department.online/pricing']],
        [['text' => '◂ Меню', 'callback_data' => 'cmd_menu']],
    ];
...
function cmdPsc($chatId, $user, $editMsgId = null) {
    ...
    $btns = [
        [['text' => '📋 Протоколы на сайте', 'url' => 'https://supply-department.online/pricing']],
        [['text' => '◂ Меню', 'callback_data' => 'cmd_menu']],
    ];
...
function cmdPlans($chatId, $user, $editMsgId = null) {
    ...
    if (!$plans) {
        $es = $entity ? ' · ' . getEntityShort($entity) : '';
        $btns = [
            [['text' => '📅 Планирование', 'url' => 'https://supply-department.online/planning']],
            [['text' => '◂ Меню', 'callback_data' => 'cmd_menu']],
        ];
...
function cmdDeliveries($chatId, $user, $editMsgId = null) {
    ...
    foreach ($orders as $i => $o) {
        ...
        $buttons[] = ['text' => "{$num}. {$o['supplier']}", 'url' => $orderUrl];
    }
    $keyboard = [];
    foreach (array_chunk($buttons, 2) as $row) { $keyboard[] = $row; }
...
function cmdSales($chatId, $user, $editMsgId = null) {
    ...
    if (!$meta || !$meta['last_date']) {
        $btns = [
            [['text' => '🏪 Реализация на сайте', 'url' => 'https://supply-department.online/analysis']],
            [['text' => '◂ Меню', 'callback_data' => 'cmd_menu']],
        ];
    ...
    $btns = [
        [['text' => '🏪 Подробный отчёт', 'url' => 'https://supply-department.online/analysis']],
        [['text' => '◂ Меню', 'callback_data' => 'cmd_menu']],
    ];
    botSend($chatId, $text, ['inline_keyboard' => $btns], $editMsgId);
}
...
function cmdSchedule($chatId, $user, $editMsgId = null, $dayNum = null) {
    ...
    if (!$all) {
        $btns = [
            [['text' => '🗓 График на сайте', 'url' => 'https://supply-department.online/schedule']],
            [$backBtn],
        ];
    ...
    if ($dayNum === null) {
        ...
        $buttons = [$row1, $row2];
        $buttons[] = [['text' => '🗓 График на сайте', 'url' => 'https://supply-department.online/schedule']];
        $buttons[] = [$backBtn];
...
function cmdAnalysis($chatId, $user, $zone = null, $page = 0, $editMsgId = null) {
    ...
    if (!$items) {
        ...
        $btns = [
            [['text' => '📊 Анализ на сайте', 'url' => 'https://supply-department.online/analysis']],
            [['text' => '◂ Меню', 'callback_data' => 'cmd_menu']],
        ];
    ...
    if (!$zone) {
        ...
        $rows[] = [['text' => '📊 Анализ на сайте', 'url' => 'https://supply-department.online/analysis']];
...
function getMenuButtons($user) {
    ...
    $buttons = [
        ...
        [['text' => '🔍 Карточки', 'callback_data' => 'cmd_cards']],
    ];
