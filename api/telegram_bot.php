...
function cmdPrices($chatId, $user, $editMsgId = null) {
    ...
    $sql = "SELECT ph.sku, p.name as product_name, ph.old_price, ph.new_price, ph.changed_by, ph.changed_at, ph.supplier
            ...
            LEFT JOIN products p ON p.id = " . productsPickIdByEntityColSql('ph.sku', 'ph.legal_entity') . "
...
function cmdToday($chatId, $user, $editMsgId = null) {
    ...
    $s = $pdo->prepare("SELECT p.name, ROUND(a.stock / (a.consumption / GREATEST(a.period_days, 1))) as days_left FROM analysis_data a LEFT JOIN products p ON p.id = " . productsPickIdByEntityColSql('a.sku', 'a.legal_entity') . " WHERE a.consumption > 0 AND a.stock > 0" . $eFilterA . " HAVING days_left <= 5 ORDER BY days_left ASC LIMIT 10");
    $s->execute($params);
...
function generatePricesCsv($entity) {
    global $pdo;
    $sql = "SELECT pp.sku, p.name, pp.supplier, pp.price, pp.vat_rate, pp.currency, pp.unit_type FROM product_prices pp LEFT JOIN products p ON p.id = " . productsPickIdByEntityColSql('pp.sku', 'pp.legal_entity') . " WHERE pp.price_type = 'purchase'";
    $params = [];
...
function searchCardDirect($chatId, $query, $userMsgId = null, $botMsgId = null) {
    ...
    if ($allSkus) {
        ...
        $params[] = $stockEntity;
        $st = $pdo->prepare("SELECT a.sku, p.name, a.stock, COALESCE(p.qty_per_box, 1) as qty_per_box FROM analysis_data a LEFT JOIN products p ON p.id = " . productsPickIdByEntityColSql('a.sku', 'a.legal_entity') . " WHERE a.sku IN ({$placeholders}) AND a.legal_entity = ? AND a.stock > 0");
        $st->execute($params);
