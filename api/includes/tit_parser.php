...
function titNormalizePlateSpacing(string $text): string
{
    ...
    // Разделитель внутри номера — пробел/таб/дефис, но НЕ перенос строки.
    // На OCR-тексте накладной \s склеивал соседние строки: «09.2022» из даты
    // и обрывок «ет» ниже превращались в «2022ет» и уходили как номер машины.
    $SEP = '[\-\t ]';
    // 1. Многосегментный letter-first: «BB-10-18-5» → «BB10185»
    $text = preg_replace_callback(
        '/(?<![\p{L}\p{N}])([' . $A . ']{1,2})' . $SEP . '+(\d{1,4})' . $SEP . '+(\d{1,4})' . $SEP . '+(\d{1,2})(?![\p{L}\p{N}])/u',
        fn($m) => $m[1] . $m[2] . $m[3] . $m[4],
        $text
    ) ?? $text;
    // 2. Letter-first 2-group: «АС 6668-5» → «АС6668-5», «AB-12345» → «AB12345»
    $text = preg_replace_callback(
        '/(?<![\p{L}\p{N}])([' . $A . ']{1,2})' . $SEP . '+(\d{4,6})((?:' . $SEP . '\d{1,2})?)(?![\p{L}\p{N}])/u',
        fn($m) => $m[1] . $m[2] . $m[3],
        $text
    ) ?? $text;
    // 3. Digits-first (новый формат): «1234 AB-7» → «1234AB-7»
    $text = preg_replace_callback(
        '/(?<![\p{L}\p{N}])(\d{4,6})' . $SEP . '+([' . $A . ']{1,2})((?:' . $SEP . '\d{1,2})?)(?![\p{L}\p{N}])/u',
        fn($m) => $m[1] . $m[2] . $m[3],
        $text
    ) ?? $text;
    ...
    $text = preg_replace_callback(
        '/(?<![\p{L}\p{N}])(\d{4})' . $SEP . '(\d{1,2})' . $SEP . '([' . $A . ']{2})(?![\p{L}\p{N}])/u',
        fn($m) => $m[1] . $m[3] . $m[2],
        $text
    ) ?? $text;
    return $text;
