...
if ($sessionUser) {
    ...
    if ($userRole !== 'admin') {
        ...
        // Уведомления — личные, а не данные модуля. Они висели на «Истории», и
        // сотрудник без этого раздела не видел вообще ни одного своего
        // уведомления, включая рассылки администратора; колокольчик молча
        // получал отказ каждые 30 секунд. Записи и так фильтруются по имени
        // получателя, поэтому чужого здесь не покажут.
...
$filterWhitelist = [
    ...
    'supplier_payments' => ['id','order_id','supplier','legal_entity','legal_entity_group','status','payment_date','created_at'],
    ...
    'search_logs'   => ['id','searcher_kind','searcher_name','restaurant_number','query','found','created_at'],
    'users'         => ['id','name','role'],
...
$writeWhitelist = [
    ...
    'plans'        => ['id','supplier','legal_entity','data','created_by','updated_by','created_at','updated_at','notes','period_type','period_count','period_frequency','truck_pallets','items','start_date','planning_date','consumption_period_days','input_unit','note'],
    ...
    'suppliers'    => ['id','short_name','full_name','legal_entity','legal_entity_group','is_active','dlt','doc','palletization','notes','schedule','whatsapp','telegram','viber','email','cc_emails','country','payment_delay_days','order_url'],
    ...
    'restaurant_sales' => ['id','sale_date','analog_group','quantity','restaurant_count','legal_entity_group'],
    ...
    'stock_collection_data' => ['id','collection_id','product_id','restaurant_number','expiry_date','stock','source','submitted_at'],
    'stock_collection_products' => ['id','collection_id','product_name','product_sku','unit','need_expiry','note','sort_order'],
...
if ($method === 'GET') {
    ...
    // Уведомления: не-админ видит ТОЛЬКО свои и общие рассылки. Раньше отбор
    // делал только фронт своим фильтром, а сервер отдавал что попросят — то
    // есть чужие уведомления читались простым запросом без фильтра.
    if ($sessionUser && $sessionUser['role'] !== 'admin' && $table === 'notifications') {
        $where[] = "(`target_user` = ? OR `type` = 'broadcast')";
        $params[] = $sessionUser['name'] ?? '';
    }
