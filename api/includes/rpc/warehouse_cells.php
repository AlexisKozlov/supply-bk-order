 ...
 */
    ...
    if ($fn === 'save_warehouse_cells') {
        ...
        try {
            ...
            // Защита от багов парсинга имени файла: дата не может быть позже завтрашнего дня.
            // Раньше регулярка extractStockReportDateFromName хватала середину ISO-даты
            // («2026-05-27» → 26-05-27 → 2027-05-26) и сервер молча принимал «будущее».
            // Потом save_warehouse_cells блокировал реальные свежие загрузки. Лучше
            // упасть прямо сейчас с понятным сообщением.
            $tomorrow = date('Y-m-d', strtotime('+1 day'));
            if ($uploadDate > $tomorrow) {
                respond([
                    'error' => "Дата файла «{$uploadDate}» в будущем. Проверьте имя файла или введите дату вручную."
                ], 400);
            }
