...
if ($method === 'POST' && $krId && $krAction === 'not-returned') {
    ...
    if ($isRestaurant) {
        ...
        // Отметить можно начиная С ДНЯ вывоза: машина не приехала — ресторан
        // сообщает сразу, а не ждёт следующего дня. Раньше проверка была
        // строгой ($today <= return_date), и в сам день вывоза кнопка не
        // работала — неудобно, отметка приходила с опозданием на сутки.
        $today = (new DateTime('now', new DateTimeZone('Europe/Minsk')))->format('Y-m-d');
        if (empty($row['return_date']) || $today < $row['return_date']) {
            krRespond(['error' => 'Отметить «не сдана» можно начиная с дня вывоза'], 422);
        }
