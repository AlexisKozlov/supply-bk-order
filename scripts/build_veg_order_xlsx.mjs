#!/usr/bin/env node
/**
 * Генератор Excel-сводки по Планете Ресторанов после дедлайна.
 *
 * Использование: node scripts/build_veg_order_xlsx.mjs <json_path> <out_xlsx_path>
 *
 * Входной JSON:
 * {
 *   "session_name": "Неделя 16 (13.04-18.04)",
 *   "delivery_date_fmt": "15.04.2026 (Ср)",
 *   "sheet_name": "Планета Ресторанов",
 *   "products": [ { "id": 12, "name": "Огурец", "unit": "kg" } ],
 *   "restaurants": [
 *     { "number": 1, "city": "Минск", "region": "Минск", "address": "...", "submitted": true, "auto": false }
 *   ],
 *   "items": {
 *     "1_12": { "qty": 10, "is_admin": false, "is_auto": false }
 *   }
 * }
 *
 * Стилизация:
 *   - обычная строка (submitted && !auto): белая
 *   - auto-строка (submitted && auto): оранжевая (подставлено из прошлой)
 *   - не подал (not submitted): красная
 */
...
if (!jsonPath || !outPath) {
    console.error('Usage: build_veg_order_xlsx.mjs <json> <out>');
    process.exit(2);
}

const data = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
const sessionName = data.session_name || 'Сессия';
const dateFmt = data.delivery_date_fmt || '';
const sheetName = (data.sheet_name || 'Планета Ресторанов').slice(0, 28);
...
// Авто-подстановка из прошлой — оранжевые
const autoQtyStyle = { font: { bold: true, sz: 11, name: 'Calibri', color: { rgb: 'E65100' } }, fill: { fgColor: { rgb: 'FFE0B2' } }, alignment: { horizontal: 'center', vertical: 'center' }, border };
const autoRestStyle = { font: { bold: true, sz: 11, name: 'Calibri', color: { rgb: 'E65100' } }, fill: { fgColor: { rgb: 'FFE0B2' } }, alignment: { horizontal: 'center', vertical: 'center' }, border };
const autoAddrStyle = { font: { sz: 10, bold: true, color: { rgb: 'E65100' }, name: 'Calibri' }, fill: { fgColor: { rgb: 'FFE0B2' } }, alignment: { horizontal: 'left', vertical: 'center' }, border };
const autoNoteStyle = { font: { sz: 10, italic: true, bold: true, name: 'Calibri', color: { rgb: 'E65100' } }, fill: { fgColor: { rgb: 'FFE0B2' } }, alignment: { horizontal: 'left', vertical: 'center' }, border };
...
for (const p of prods) {
    const unit = p.unit === 'pcs' ? 'шт' : 'кг';
    header.push(`${p.name}\n(${unit})`);
}
header.push('Пометка');

const titleRow = [`${sessionName} — поставка ${dateFmt}`];
const aoa = [titleRow, header];
const rowMeta = [];
const merges = [{ s: { r: 0, c: 0 }, e: { r: 0, c: header.length - 1 } }];

function getQty(r, p) {
    const v = items[`${r.number}_${p.id}`];
    return v || null;
}

function rowNoteFor(r) {
    if (!r.submitted) return 'Не подал';
    if (r.auto) return 'Авто (прошлая)';
    return '';
}
...
for (const city of cityOrder) {
    ...
    for (let i = 0; i < group.length; i++) {
        ...
        row.push(rowNoteFor(r));
        aoa.push(row);
        rowMeta.push({ type: 'data', row: r, isEven: i % 2 === 1, isSubmitted: !!r.submitted, isAuto: !!r.auto });
    }
...
for (let mi = 0; mi < rowMeta.length; mi++) {
    ...
    if (meta.type === 'city') {
        ...
        if (numCell) {
            if (!meta.isSubmitted) numCell.s = missRestStyle;
            else if (meta.isAuto) numCell.s = autoRestStyle;
            else numCell.s = { ...restStyle, ...(meta.isEven ? { fill: evenRowBg } : {}) };
        }
        const aCell = ws[XLSX.utils.encode_cell({ r, c: 1 })];
        if (aCell) {
            if (!meta.isSubmitted) aCell.s = missAddrStyle;
            else if (meta.isAuto) aCell.s = autoAddrStyle;
            else aCell.s = { ...addrStyle, ...(meta.isEven ? { fill: evenRowBg } : {}) };
        }
        for (let pi = 0; pi < prods.length; pi++) {
            ...
            if (meta.isAuto) { cell.s = autoQtyStyle; continue; }
        ...
        if (nCell) {
            if (!meta.isSubmitted) nCell.s = missNoteStyle;
            else if (meta.isAuto) nCell.s = autoNoteStyle;
            else nCell.s = { ...noteStyle, ...(meta.isEven ? { fill: evenRowBg } : {}) };
        }
